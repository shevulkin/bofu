...
class Schema
{
    ...
    public static function upgrade(): void
    {
        ...
        if ($ver < 35) {
            // Фіскальні чеки «Вчасно.Каса».
            //
            // Токен належить КАСІ, а каса — торговій точці: гроші отримує
            // конкретний магазин своїм ПРРО, і фіскальний номер зі зміною
            // належать саме йому. Тому токен лежить у точці, а не лише в
            // загальних налаштуваннях; порожній означає «працює на загальній
            // касі» (у більшості це один ФОП на всі точки).
    ...
    public static function tables(): array
    {
        return [
            ...
            'stores' => [
                ...
                // Каса «Вчасно» цієї точки. Токен належить касі, а каса —
                // точці: чек мусить пробитись саме там, де стоїть покупець.
                // Порожньо — точка працює на загальній касі з налаштувань.
             ...
             *
             * status: pending — відповіді не було (чек МІГ пробитись, повторимо
             * тим самим tag), done — є фіскальний номер, error — ПРРО відмовив
             * по суті і без людини не обійтись.
             */
            'fiscal_receipts' => [
                'id' => 'id',
                'order_id' => 'int',                 // підзамовлення (parent_id IS NOT NULL)
                ...
                'type' => "str default 'sell'",      // sell|return
                ...
                'status' => "str default 'pending'", // pending|done|error
