...
«Усі фасовки разом» — свічки різного кольору, банки різного обʼєму, набори одного й того самого. Три плюс дві дають пʼять, і поріг «від 5 шт» спрацьовує. Так стоїть у більшості товарів.

«Кожну фасовку окремо» — коли фасовки насправді різні речі: розміри шапки, розміри костюма. Пʼять різних розмірів — це пʼять різних речей, і кожна йде своїм рахунком.
    ...
    <div class="row-list" id="variantRows">
      <?php foreach ($variants as $v): $vid = (int)$v['id']; $opts = $variant_options[$vid] ?? []; ?>
        <div class="grid-row variant-row" data-vid="<?= $vid ?>">
          ...
          <?php /* Своя стеля знижки: дрібна фасовка витримує менший відсоток, ніж ящик */ ?>
          <input type="text" name="variant[<?= $vid ?>][max_discount]" value="<?= e(num_val($v['max_discount'] ?? null)) ?>"
                 placeholder="стеля, %" title="Найбільша сумарна знижка на цю фасовку. Порожньо — стеля товару">
