  ...
  <?php foreach ($orders as $o): $id = (int)$o['id'];
    ...
    <tr<?= $foreign ? ' class="row-foreign"' : '' ?>>
      ...
      <td><span class="status-pill st-<?= e($o['status']) ?>"><?= e($statuses[$o['status']] ?? $o['status']) ?></span>
        <?php /* Оплата карткою — під статусом, і лише коли про неї є що сказати.
                 «Чекає оплати» тут головніше за статус: таке замовлення не
                 треба ні збирати, ні везти, доки гроші не прийшли, і
                 переплутати його з новим оплаченим коштує зібраної посилки. */ ?>
        <?php if ((string)($o['payment_kind'] ?? '') === 'card' && !$o['parent_id']): ?>
          <div style="margin-top:5px;white-space:nowrap;font-size:12px"
               class="<?= trim((string)($o['paid_at'] ?? '')) !== '' ? '' : 'dim' ?>">
            <?= trim((string)($o['paid_at'] ?? '')) !== '' ? '💳 оплачено' : '💳 чекає оплати' ?>
          </div>
        <?php endif; ?>
        <?php
          /*
