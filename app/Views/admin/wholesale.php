<?php
/**
 * @var array $categories розділи для вибору яруса
 * @var int   $tier_cat   ярус, який редагують (0 — загальна шкала)
...
Заповнена шкала і шкала, що діє, — різні речі. Ярус нижче перебиває верхній цілком: якщо на всіх товарах розділу задані власні шкали, шкала самого розділу не подіє ні на що, хоч і лишиться на місці.
...
Рахується тим самим кодом, що вирішує ціну в кошику, тож розбіжності між цією таблицею й тим, що заплатить покупець, бути не може.">
  ...
  <?php if (!$ladders): ?>
    <p class="dim" style="margin:0">Жодної шкали ще немає — опт не спрацює ні на чому.
      Задайте загальну або шкалу розділу нижче.</p>
  <?php else: ?>
    <table class="tbl">
      <tr>
        <th style="width:34%">Ярус</th>
        <th>Шкала</th>
        <th class="num" data-help-title="Колонка «Діє на»"
            data-help="Скільки активних товарів беруть саме цю шкалу.

Нуль означає, що ярус перебито нижчим: шкала заповнена, але жоден товар до неї не доходить. Це не помилка — так буває, коли всім товарам розділу задали власні шкали, — але й користі з такої шкали немає, поки в розділ не додадуть новий товар.">Діє на</th>
        <th class="col-mid"></th>
      </tr>
    ...
    <?php if ($idle): ?>
      <p class="dim" style="margin:12px 0 0">
        Приглушені рядки нічого не роблять: їх перебито ярусом нижче.
